...
* **Judger Skill**: install `vllm` in the same environment that runs Judger. Judger starts the local vLLM OpenAI-compatible API server itself, using the current interpreter (`sys.executable`) on port 8911, and exits with an error if `vllm` cannot be imported. The model comes from `judger.eval_model_path`; `eval_model_name`, `eval_vllm_tensor_parallel_size`, `eval_vllm_gpu_memory_utilization`, and `cuda_visible_devices` tune it. When `eval_model_name` is empty the served model name defaults to the last segment of the model path, and the same name is passed to vLLM as `--served-model-name`.
