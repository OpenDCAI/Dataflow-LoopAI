<template>
    <div class="webcrawler-log-panel">
...
const props = defineProps({
    ...
    graphClsPrefix: {
        type: String,
        default: 'WebCrawlerAgent'
    }
})

// 定义节点顺序
const nodeOrder = ['start_node', 'crawl_node', 'webcrawler_dataset_node', 'end_node']

// 节点显示名称映射
const nodeNameMap = {
    'start_node': '初始化配置',
    'crawl_node': '网页爬取',
    'webcrawler_dataset_node': '数据集处理',
    'end_node': '完成'
}
...
const orderedNodes = computed(() => {
    ...
    if (!info) {
        return nodeOrder.map(nodeName => ({
            key: `${props.graphClsPrefix}.${nodeName}`,
            nodeName: nodeName,
            value: null
        }))
    }
    ...
    return nodeOrder.map(nodeName => {
        const key = `${props.graphClsPrefix}.${nodeName}`
        return {
            key: key,
            nodeName: nodeName,
            value: info[key] || null
