...
class LoopAIState(MessagesState):
    ...
    configer_error: str  # the errors of configer, it can indicate the LLM to summarize the configer statement
    ...
    next_to: str  # defined the next node to be executed, usually is used by the feedback node, the value of node name should exists in the current graph.
