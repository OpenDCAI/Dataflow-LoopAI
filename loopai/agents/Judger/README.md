...
### 2️⃣ vllm_start_node（vllm启动）

🟢 输入
- eval_vllm_command【str】：vllm服务启动命令。例如`"python -m vllm.entrypoints.openai.api_server --model /root/brjverl/models/Qwen2.5-Coder-7B-Instruct/ --port 8911 --tensor-parallel-size 1 --trust-remote-code --gpu-memory-utilization 0.9 --enable-auto-tool-choice --tool-call-parser hermes"`。单行启动命令字符串。
...
🟡 功能
- 如果这两个配置参数存在空值，则认为是用户已提前启动了vllm，并进行了相关配置。否则将调用这两项配置启动vllm。
	
### 3️⃣ data_format_node（格式匹配）
