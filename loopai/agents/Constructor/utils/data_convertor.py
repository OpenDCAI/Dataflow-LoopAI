...
class DataConvertor:
    ...
    async def invoke_data_mapping(
        ...
        """Invoke LLM for data mapping with triple verification
        
        Performs three independent mapping calls with random sampling, 
        and only returns result if all three are consistent.
        """
        logger.info("Starting data mapping with triple verification...")
        
        # If dataset is available, perform triple verification
        if dataset is not None:
            return await self._invoke_data_mapping_with_verification(
            ...
            # For single record case, perform single mapping (no verification needed)
            return await self._single_invoke_data_mapping(
                column_names=column_names,
                sample_record=sample_record,
                dataset=None,
                user_target=user_target,
    ...
    async def _invoke_data_mapping_with_verification(
        self,
        column_names: List[str],
        sample_record: Dict[str, Any],
        dataset: Any,
        ...
        """Perform triple verification of data mapping"""
        verification_results = []
        
        # Perform three independent mapping calls
        for attempt in range(3):
            logger.info(f"Data mapping verification attempt {attempt + 1}/3...")
            try:
                result = await self._single_invoke_data_mapping(
                    column_names=column_names,
                    sample_record=sample_record,
                    dataset=dataset,
                    user_target=user_target,
                ...
                raise ValueError(f"Data mapping verification attempt {attempt + 1} failed: {e}")
        
        # Compare results
        if not self._compare_mapping_results(verification_results, category):
            logger.error("Triple verification failed: results are inconsistent")
            logger.debug(f"Result 1: {json.dumps(verification_results[0], ensure_ascii=False, indent=2)}")
            logger.debug(f"Result 2: {json.dumps(verification_results[1], ensure_ascii=False, indent=2)}")
            logger.debug(f"Result 3: {json.dumps(verification_results[2], ensure_ascii=False, indent=2)}")
            raise ValueError("Data mapping verification failed: three attempts returned inconsistent results")
        
        logger.info("Triple verification passed: all three results are consistent")
        return verification_results[0]
    
    def _compare_mapping_results(self, results: List[Dict[str, Any]], category: str) -> bool:
        """Compare mapping results for consistency
        
        For SFT mode: focuses on messages structure and mapping
        For PT mode: focuses on text field mapping
        """
        if len(results) < 2:
            return True
        ...
        if category_upper == "SFT":
            # For SFT, focus on messages mapping
            return self._compare_sft_mapping_results(results)
        elif category_upper == "PT":
            # For PT, focus on text field mapping
            return self._compare_pt_mapping_results(results)
        else:
            # Unknown category, do basic comparison
            return results[0] == results[1] == results[2]
    
    def _compare_sft_mapping_results(self, results: List[Dict[str, Any]]) -> bool:
        """Compare SFT mapping results, focusing on messages structure"""
        if len(results) < 2:
            return True
        
        # Extract messages from each result
        messages_list = [r.get('messages', []) for r in results]
        
        # Check if all have same number of messages
        if len(set(len(m) for m in messages_list)) != 1:
            logger.warning(f"Messages count mismatch: {[len(m) for m in messages_list]}")
            return False
        
        # Compare each message's role and content mapping
        for i in range(len(messages_list[0])):
            base_msg = messages_list[0][i]
            for other_messages in messages_list[1:]:
                if i >= len(other_messages):
                    return False
                other_msg = other_messages[i]
                
                # Compare role
                if base_msg.get('role') != other_msg.get('role'):
                    logger.warning(f"Message {i} role mismatch: {base_msg.get('role')} vs {other_msg.get('role')}")
                    return False
                
                # Compare content mapping (field specification)
                base_content = base_msg.get('content')
                other_content = other_msg.get('content')
                
                # Normalize content spec for comparison
                base_content_normalized = self._normalize_field_spec(base_content)
                other_content_normalized = self._normalize_field_spec(other_content)
                
                if base_content_normalized != other_content_normalized:
                    logger.warning(f"Message {i} content mapping mismatch: {base_content} vs {other_content}")
                    return False
        
        # System field comparison (optional)
        system_list = [r.get('system') for r in results]
        if len(set(str(s) for s in system_list if s is not None)) > 1:
            logger.warning(f"System field mismatch: {system_list}")
            return False
        
        return True
    
    def _compare_pt_mapping_results(self, results: List[Dict[str, Any]]) -> bool:
        """Compare PT mapping results, focusing on text field mapping"""
        if len(results) < 2:
            return True
        
        # Extract text field from each result
        text_fields = [r.get('text') for r in results]
        
        # Normalize text field specs for comparison
        normalized_texts = [self._normalize_field_spec(t) for t in text_fields]
        
        if len(set(normalized_texts)) != 1:
            logger.warning(f"Text field mapping mismatch: {text_fields}")
            return False
        
        return True
    
    def _normalize_field_spec(self, field_spec: Any) -> str:
        """Normalize field specification for comparison"""
        if field_spec is None:
            return ""
        if isinstance(field_spec, list):
            # Sort list for comparison
            sorted_list = sorted(str(f) for f in field_spec)
            return json.dumps(sorted_list, sort_keys=True)
        return str(field_spec).strip()
    
    async def _single_invoke_data_mapping(
        self, 
        column_names: List[str], 
        sample_record: Dict[str, Any], 
        dataset: Any = None,
        ...
        """Single invocation of LLM for data mapping (internal method)"""
        logger.debug("Building data mapping prompt messages...")
        ...
        try:
            ...
            if dataset is not None:
                # Randomly sample 3 records for each verification attempt
                sampled_records = await self._sample_random_records(dataset, num_samples=3)
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                logger.debug(f'LLM data mapping response received (attempt {attempt}): {answer_text[:200]}...')
                ...
                logger.debug(f"Data mapping result parsed successfully: {annotation_result}")
    ...
    async def _sample_random_records(self, dataset: Any, num_samples: int = 3) -> List[Dict[str, Any]]:
        """Randomly sample records from dataset (ensures different samples each call)"""
        def _get_dataset_size():
            return len(dataset)
        ...
        if dataset_size <= actual_samples:
            ...
            # Random sampling ensures different samples each call
            sample_indices = random.sample(range(dataset_size), actual_samples)
        
        # Sample records concurrently
        async def _sample_single_record(idx: int):
            record = await asyncio.to_thread(_get_record, idx)
            truncated_record = {k: self._truncate_value(v) for k, v in record.items()}
            return truncated_record
        ...
        logger.debug(f"Randomly sampled {len(sampled_records)} records from dataset (total: {dataset_size})")
        return sampled_records
...
Please identify which files are data files that should be processed. Return a JSON array of file paths."""
    ...
    async def invoke_file_filter(
        ...
        if self.prompt_loader:
            try:
                system_prompt = self.prompt_loader("system", "file_filter_prompt")
                task_prompt = self.prompt_loader("task", "file_filter_prompt")
                ...
                system_prompt = self._get_default_file_filter_system_prompt()
                human_prompt = self._get_default_file_filter_task_prompt(
                    file_path, sampled_records, dataset_background
                )
        else:
            system_prompt = self._get_default_file_filter_system_prompt()
            human_prompt = self._get_default_file_filter_task_prompt(
        ...
        try:
            ...
            try:
                ...
                if isinstance(result, dict):
                    ...
                    is_match = "true" in answer_text.lower() or "yes" in answer_text.lower() or "符合" in answer_text or "匹配" in answer_text
            ...
            # Default to keeping the file if LLM call fails
            logger.warning(f"File filter LLM call failed, keeping file by default: {file_path}")
            return True

    def _get_default_file_filter_system_prompt(self) -> str:
        """Get default file filter system prompt"""
        return """You are a dataset filtering expert. Your task is to determine whether a data file matches the given dataset background description.

You will be provided with:
1. A file path
2. Three sampled records from the file
3. A dataset background description

You need to determine if the content of the file matches the dataset background. Return a JSON object with a boolean field "is_match" indicating whether the file matches the background."""

    def _get_default_file_filter_task_prompt(
        self,
        file_path: str,
        sampled_records: List[Dict[str, Any]],
        dataset_background: str
    ) -> str:
        """Get default file filter task prompt"""
        sampled_records_str = json.dumps(sampled_records, indent=2, ensure_ascii=False)
...
Sampled records from the file (3 records):
{sampled_records_str}

Please analyze whether the content of this file matches the dataset background description. Return a JSON object with a boolean field "is_match" (true if matches, false if not)."""
