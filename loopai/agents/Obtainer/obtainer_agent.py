...
class ObtainerAgent(BaseAgent):
        ...
        def start_node(state: LoopAIState):
            ...
            if user_query:
                ...
                if user_query:
                        ...
                        if model_name and base_url and api_key:
                            ...
                            if isinstance(classification_result, dict):
                                ...
                                if dataset_background:
                                    state["obtainer_datasets_background"] = dataset_background
                                    logger.info(f"Extracted dataset background: {dataset_background[:100]}...")
                                else:
                                    # Use user_query as fallback dataset_background
                                    state["obtainer_datasets_background"] = user_query if user_query else objective
                    ...
                    state["obtainer_datasets_background"] = ""
            ...
            if state.get("obtainer_category"):
            ...
            if "obtainer_max_download_subtasks" not in state:
                state["obtainer_max_download_subtasks"] = 5  # 每个子任务最多5个下载子任务
            
            # 设置递归上限为50，支持更复杂的图结构
            # recursion_limit 会在调用图时通过 config 参数传递
            if "obtainer_recursion_limit" not in state:
                state["obtainer_recursion_limit"] = 50
    ...
    def get_task_decomposer_node(self):
        ...
        def task_decomposer_node(state: LoopAIState):
            ...
            if state.get("obtainer_task_list") and len(state.get("obtainer_task_list", [])) > 0:
                logger.info(f"Task list already exists with {len(state['obtainer_task_list'])} tasks, skipping decomposition")
                return state
            ...
            if not user_input:
                ...
                state["obtainer_task_list"] = [{"task_name": "收集数据集用于大模型微调"}]
                state["obtainer_current_task_index"] = 0
                state["automated_query"] = state["obtainer_task_list"][0]["task_name"]
                return state
            ...
            try:
                ...
                base_url = state.get("obtainer_base_url") or state.get("analyze_base_url")
                api_key = state.get("obtainer_api_key") or state.get("analyze_api_key")
                temperature = state.get("obtainer_temperature", 0.3)
                ...
                if model_name and base_url and api_key:
                    ...
                    state["obtainer_task_list"] = task_list
                    state["obtainer_current_task_index"] = 0
                    ...
                    if task_list and len(task_list) > 0:
                        ...
                        try:
                            ...
                            if isinstance(classification_result, dict):
                                ...
                                # else:
                                    # state["obtainer_datasets_background"] = first_task
                            ...
                            state["obtainer_datasets_background"] = first_task
                            logger.info(f"First subtask category (fallback): {state['obtainer_category']}")
                    else:
                        logger.warning("Task decomposition returned empty list, using original input")
                        state["obtainer_task_list"] = [{"task_name": user_input}]
                        state["obtainer_current_task_index"] = 0
                        ...
                        if model_name and base_url and api_key:
                            try:
                                ...
                                if isinstance(classification_result, dict):
                                    ...
                                    # dataset_background = classification_result.get("dataset_background", "")
                                    state["obtainer_category"] = category
                                    # state["obtainer_datasets_background"] = dataset_background if dataset_background else user_input
                                else:
                                    category = classification_result if isinstance(classification_result, str) else "PT"
                                    state["obtainer_category"] = category
                                    # state["obtainer_datasets_background"] = user_input
                            except Exception as e:
    ...
    def check_next_task_node(state: LoopAIState):
        ...
        if current_index < len(task_list):
            ...
            if "obtainer_category" in state:
                del state["obtainer_category"]
    ...
    def next_task_node(state: LoopAIState):
        ...
        if current_index < len(task_list):
            ...
            state["obtainer_research_summary"] = ""
            state["obtainer_subtasks"] = []
            state["obtainer_urls_visited"] = []
                ...
                if model_name and base_url and api_key:
                    ...
                    if isinstance(classification_result, dict):
                        ...
                        # else:
                            ...
                            # state["obtainer_datasets_background"] = task_name
            ...
            state["obtainer_current_task_index"] = current_index + 1
