...
def _analyze_user_intent(state: LoopAIState, user_input: str, store: BaseStore = None) -> LoopAIState:
    ...
    if not user_input or not user_input.strip():
        state["obtainer_mapping_user_intent"] = "unclear"
    ...
    if user_input_lower == "list":
        state["obtainer_mapping_user_intent"] = "list_formats"
    ...
    for format_id in PRESET_FORMATS.keys():
        if format_id in user_input_lower:
            state["obtainer_mapping_user_intent"] = "preset_format"
            state["obtainer_mapping_selected_format_id"] = format_id
...
Available preset format IDs: {preset_format_ids}"""
        ...
        if result:
            ...
            state["obtainer_mapping_user_intent"] = action
            
            if action == "preset_format" and format_id:
                state["obtainer_mapping_selected_format_id"] = format_id.lower()
            elif action == "custom_format":
                state["obtainer_mapping_custom_description"] = custom_desc or user_input
            
            logger.info(f"LLM analysis result: action={action}, format_id={format_id}")
        else:
            state["obtainer_mapping_user_intent"] = "custom_format"
...
def _save_to_store(state: LoopAIState, store: BaseStore):
    ...
    try:
        ...
        data = {
            ...
            "user_intent": state.get("obtainer_mapping_user_intent", ""),
            "selected_format_id": state.get("obtainer_mapping_selected_format_id", ""),
            "custom_description": state.get("obtainer_mapping_custom_description", "")[:200]
        }
