...
from .tools import create_data_load_tool, create_file_read_tool, create_web_search_tool
...
def _build_system_prompt(category: str) -> str:
    ...
    schema_str = DatasetMappingPlan.json_schema_str()
    category_desc = "预训练——将文本字段拼接为连续文本" if category == "PT" else "监督微调——构建多轮对话消息"
    return template.replace("{mapping_plan_schema}", schema_str).replace(
...
def _extract_mapping_plan(messages: List[BaseMessage], dataset_name: str, category: str) -> Optional[DatasetMappingPlan]:
    ...
    for msg in reversed(messages):
        ...
        for candidate in candidates:
            try:
                ...
                return DatasetMappingPlan.model_validate(data)
            except Exception:
                continue
    return None
...
class DatasetAgent:
    ...
    def _agent_node(self, state: dict) -> dict:
        ...
        response = self._bound_llm.invoke([SystemMessage(content=system_prompt)] + messages)
    ...
    async def run(self) -> DatasetAgentResult:
        ...
        mapping_plan = _extract_mapping_plan(all_messages, ds, self.category)
