...
# code bench 的 ``format_type`` 只支持这两种写法，直接对应 evalplus 的两个数据集
# （也是官方镜像里烤好的那两份）。不做别名：名字写错就当场报错，不猜。
FORMAT_TYPES = {"humaneval+": "humaneval", "mbpp+": "mbpp"}
...
def resolve_code_task(judger: Dict[str, Any]) -> str:
    """bench 的 ``format_type`` → evalplus 数据集名，只认 ``humaneval+`` / ``mbpp+``。"""
...
def _ensure_image(writer=None) -> None:
    ...
    用 ``CODE_EVAL_IMAGE`` 指向本地已有镜像），省得再去翻文档。
    """
    try:
        probe = subprocess.run(
            ["docker", "image", "inspect", CODE_EVAL_IMAGE],
            stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=False)
    except FileNotFoundError as exc:
        emit_error(
            exc, code=ErrorCode.DEPENDENCY_ERROR, recoverable=True, stream_writer=writer,
            message="code 判分要在宿主机上跑 evalplus 官方镜像，但当前环境没有 docker 命令；"
    ...
    if writer:
        ...
        writer(StreamEvent(current="judger", progress=0.0,
                           message=f"本地没有 {CODE_EVAL_IMAGE}，正在拉取",
                           data={"image": CODE_EVAL_IMAGE}))
    try:
        subprocess.run(["docker", "pull", CODE_EVAL_IMAGE], check=True,
        ...
        emit_error(
            ...
            message=f"拉取 evalplus 官方镜像失败：{CODE_EVAL_IMAGE}（{reason}）。"
                    "补救办法任选其一："
                    f"1) 在能联网的机器上 `docker pull {CODE_EVAL_IMAGE}`，再 "
                    f"`docker save {CODE_EVAL_IMAGE} | gzip > evalplus.tgz` 拷到本机，"
                    "执行 `docker load -i evalplus.tgz`；"
                    "2) 用环境变量 CODE_EVAL_IMAGE 指向本机已有的等价镜像；"
                    "3) 换一台已有该镜像的机器重跑。")
...
def dataset_dump_hint(task: str, target: str = "") -> str:
...
def check_problem_file(problem_path: str, task: str) -> List[str]:
    """检查题目文件是不是 evalplus 的 ``task`` 数据集格式；返回问题列表（空 = 通过）。

    只做"这份数据能不能判分"的检查：必需字段、task_id 前缀、题目数量。分开写是因为
    HumanEval+ 和 MBPP+ 的字段不完全一样（``test`` vs ``assertion``），混用一套
    字段表就会给出误导性的报错。
    """
    if task not in _PROBLEM_FIELDS:
