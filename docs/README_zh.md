...
* **Judger Skill**：`vllm` 需要装在运行 Judger 的同一个环境里。Judger 自己启动本地 vLLM OpenAI 兼容服务，用当前解释器（`sys.executable`）监听 8911 端口，导入不到 `vllm` 会直接报错退出。模型由 `judger.eval_model_path` 指定，另可用 `eval_model_name`、`eval_vllm_tensor_parallel_size`、`eval_vllm_gpu_memory_utilization`、`cuda_visible_devices` 调整。`eval_model_name` 留空时，对外模型名取模型路径的最后一段，并以同一个值传给 vLLM 的 `--served-model-name`。
