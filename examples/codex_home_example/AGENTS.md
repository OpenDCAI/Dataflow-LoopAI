...
- `judge` 优先使用 `judger_run`
- 用户要求查看 Judger 过程或评测明细时，优先使用 `judger_load_events`
- `train` 优先使用 `trainer_run`
- 用户要求查看 Trainer 过程或训练事件时，优先使用 `trainer_load_events`
...
- 判断 `loopai_mcp` 是否可用，必须以当前会话是否存在可调用的 `*` tools 为准
- 如果用户明确要求“通过 MCP 执行评测 / 调用 `judger_run`”，必须调用 `judger_run`
- 如果用户明确要求走 MCP，而当前会话中不存在 `judger_run`，必须直接说明“当前会话未加载该 MCP tool”，禁止静默回退到 Python 直接调用 `loopai.skills.Judger`
- 同理，如果用户明确要求通过 MCP 读取 Judger 事件，必须调用 `judger_load_events`
- 只有在用户没有要求 MCP，且当前会话确实没有对应 MCP tool 时，才允许回退到本地 Python skill / 函数调用
- 只有在当前回合中实际发生了 `judger_run` 或 `judger_load_events` 的 tool call，才算“已经通过 MCP 调用了 Judger”
...
- 如果当前会话缺少 `judger_run`，但用户仍要求通过 MCP 调用，则优先使用 Python MCP SDK 作为 MCP client 连接 FastAPI 暴露的 `http://127.0.0.1:<api_port>/mcp`，而不是手写 JSON-RPC
- 这种 Python SDK 方式的标准入口是 [call_loopai_mcp.py](./examples/scripts/call_loopai_mcp.py)
- 允许的 MCP fallback 形态是：通过 `mcp.ClientSession` + HTTP MCP client 调用 server 的 `list_tools` / `call_tool`
- 不允许临时手写 initialize / tools/list 的 JSON-RPC 报文，不允许通过任何方式自行拉起独立 MCP server
...
- 如果用户明确要求通过 MCP 调用 Configer，而当前会话中不存在 `configer_*` tools，必须直接说明当前会话未加载对应 MCP tools，不能伪装成已经走了 MCP
- 只有在当前回合中实际发生了 `configer_*` 的 tool call，才算“已经通过 MCP 调用了 Configer”
...
- 如果当前会话缺少 `configer_*` wrapper tool，但用户仍要求通过 MCP 调用 Configer，也应优先使用 Python MCP SDK client 连接 FastAPI 的 `/mcp` 端点，而不是手写协议或直接 import tool 函数
