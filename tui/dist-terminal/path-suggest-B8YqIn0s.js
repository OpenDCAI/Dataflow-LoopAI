import { i as importNodeModule, n as normalizePath, r as resolvePath, t as isAbsolutePath } from "./path-CeN9rSfe.js";
//#region ../../vue-tui/src/core/path-suggest.ts
function lower(s) {
	return s.toLowerCase();
}
function normalizePathInputSeparators(input) {
	let out = "";
...
function stripLeadingDotSlash(input) {
	let i = 0;
	while (i + 1 < input.length && input.charCodeAt(i) === 46 && input.charCodeAt(i + 1) === 47) i += 2;
...
function normalizeRelativePathForIgnore(input) {
	return stripTrailingSlashes(collapseSlashes(stripLeadingSlashes(stripLeadingDotSlash(normalizePathInputSeparators(input)))));
}
...
async function suggestPaths$1(options) {
	...
	if (!rawWorkspace || rawWorkspace.includes("\0") || rawInput.includes("\0")) return {
		baseDirAbs: resolvePath(normalizePath(rawWorkspace || "."), "."),
		dirPrefix: "",
		query: "",
		suggestions: []
	};
	...
	pruneScored();
	scored.sort(cmpScored);
	const suggestions = (max ? scored.slice(0, max) : []).map((s) => {
...
//#endregion
//#region ../../vue-tui/src/cli/path-suggest.ts
var DEFAULT_IGNORED_SEGMENTS = /* @__PURE__ */ new Set([
	".git",
	"node_modules",
	"dist",
	"cache",
...
var EMPTY_GITIGNORE_RULES = [];
var gitignoreRulesCache = /* @__PURE__ */ new Map();
var gitignoreRulesLoading = /* @__PURE__ */ new Map();
function isNodeLike() {
...
function loadGitignoreRulesAsync(workspaceAbs) {
	...
	return (async () => {
		...
		try {
			...
			return compileGitignoreRules(await fs.readFile(gitignoreAbs, "utf8"));
		} catch {
...
async function loadGitignoreRules(workspaceAbs) {
	...
	return gitignoreRulesLoading.get(key) ?? startGitignoreLoad(key);
}
function peekGitignoreRules(workspaceAbs) {
	const key = normalizePath(workspaceAbs);
	const cached = gitignoreRulesCache.get(key);
	if (cached) return cached;
	startGitignoreLoad(key);
	return EMPTY_GITIGNORE_RULES;
}
...
function compileGitignoreRules(content) {
	...
	for (const rawLine of lines) {
		...
		if (!line) continue;
		const rule = line.includes("/") ? compilePathRule(line, {
			negative,
			directoryOnly
		}) : compileSegmentRule(line, {
			negative,
			directoryOnly
		});
...
function shouldIgnoreByDefault(normalizedRelPath) {
	...
	for (const seg of segments) if (DEFAULT_IGNORED_SEGMENTS.has(seg)) return true;
	return false;
}
function isGitignored(normalizedRelPath, opts) {
	...
	for (const rule of rules) {
		if (rule.kind === "segment") {
			if ((rule.directoryOnly ? opts.isDir ? segments : segments.slice(0, -1) : segments).some((s) => rule.regex.test(s))) ignored = !rule.negative;
			continue;
		}
		if ((rule.directoryOnly ? opts.isDir ? prefixes : prefixes.slice(0, -1) : prefixes).some((p) => rule.regex.test(p))) ignored = !rule.negative;
	}
	return ignored;
}
async function suggestPaths(options) {
	const workspaceAbs = normalizePath(String(options.workspaceAbs ?? ""));
	const gitignoreRules = (options.gitignore ?? "blocking") === "nonBlocking" ? peekGitignoreRules(workspaceAbs) : await loadGitignoreRules(workspaceAbs);
	return suggestPaths$1({
		...options,
		workspaceAbs,
		shouldIgnore: ({ normalizedRelPath, isDir }) => shouldIgnoreByDefault(normalizedRelPath) || isGitignored(normalizedRelPath, {
			isDir,
...
function resolveUserPath(workspaceAbs, input) {
	...
	return resolveUserPath$1(workspaceAbs, input, { homeDir: String(env.HOME || env.USERPROFILE || "") });
}
//#endregion
export { resolveUserPath, suggestPaths };
