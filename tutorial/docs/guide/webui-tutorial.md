...
打开任务面板后，先创建一个新任务，然后点击“运行”。

任务启动后，首先进入的是 `StarterAgent`。它是整个系统的主管 Agent，主要负责：
...
因此第一次进入任务时，不要急着把它当成某个具体执行器。它更像是调度和总控入口。

## 第四步：先配置全局参数

进入任务后，建议先去 UI 里的 `Config` 面板配置全局参数。
...
配置完成后，回到任务面板，就可以通过对话驱动 LoopAI 进入不同 Agent。
