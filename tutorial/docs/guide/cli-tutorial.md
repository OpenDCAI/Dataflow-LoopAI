...
入口脚本：`examples/scripts/run_trainer.py`。它会调用 Trainer Skill 内部训练子图，完整跑一遍「数据检查 → 配置生成 → 训练执行」三个节点。
...
- **只想验证模板与字段，不真训**：注释掉 `graph.invoke(...)` 那段，直接调用 `from loopai.skills.Trainer.utils.config_generator import ConfigGenerator`，单独跑配置生成，几秒内就能产出 YAML，方便检查参数是否合理。
